import { LoanApplicationStatus, LoanStatus, MemberStatus, PaymentMethod, Role, TransactionCategory, TransactionType, } from "@prisma/client";
import { prisma as defaultPrisma } from "./server.db.js";
import { AppError, asNumber, formatDateOnly } from "./server.utils.js";
const ACTIVE_LOAN_STATUSES = [LoanStatus.ACTIVE, LoanStatus.DELINQUENT];
const createDateStart = (value) => {
    const [year, month, day] = value.split("-").map(Number);
    return new Date(year, month - 1, day, 0, 0, 0, 0);
};
const createDateEnd = (value) => {
    const [year, month, day] = value.split("-").map(Number);
...
const diffInDays = (date, compareTo = new Date()) => {
    const target = new Date(date.getFullYear(), date.getMonth(), date.getDate()).getTime();
    const compare = new Date(compareTo.getFullYear(), compareTo.getMonth(), compareTo.getDate()).getTime();
    return Math.max(0, Math.floor((compare - target) / (1000 * 60 * 60 * 24)));
};
const containsQuery = (...values) => (query) => {
    if (!query)
        return true;
    const normalized = query.trim().toLowerCase();
    return values.some((value) => value?.toLowerCase().includes(normalized));
};
const toMemberStatusLabel = (status) => status === MemberStatus.ACTIVE ? "Aktif" : "Nonaktif";
const toLoanStatusLabel = (status) => {
...
const toPaymentMethodLabel = (method) => method === PaymentMethod.TRANSFER ? "Transfer" : "Tunai";
const toSavingsTypeLabel = (type, productName) => {
    const normalizedName = productName?.toLowerCase() ?? "";
    if (normalizedName.includes("pokok"))
        return "Simpanan Pokok";
    if (normalizedName.includes("wajib"))
        return "Simpanan Wajib";
    if (normalizedName.includes("sukarela"))
        return "Simpanan Sukarela";
    return type === TransactionType.MANDATORY_SAVING_DEPOSIT ? "Simpanan Wajib" : "Simpanan Sukarela";
};
const toTransactionLabel = (type) => {
    if (type === TransactionType.MANDATORY_SAVING_DEPOSIT)
        return "Setoran Wajib";
    if (type === TransactionType.VOLUNTARY_SAVING_DEPOSIT)
        return "Setoran Sukarela";
    return "Angsuran Pinjaman";
};
const toTransactionCategory = (category) => (category === TransactionCategory.SAVINGS ? "simpanan" : "pinjaman");
    ...
    status: item.status === "NEW"
        ...
        : item.status === "UNDER_REVIEW"
            ...
            : item.status === "APPROVED"
                ? "Disetujui"
                : "Ditolak",
    estimatedInstallment: asNumber(item.estimatedInstallment),
    reviewNote: item.reviewNote,
});
const getAdminUser = async (prisma, userId) => {
...
const buildCashflowItems = async (prisma) => {
    ...
    const incomingItems = transactions.map((item) => ({
        code: item.transactionCode,
        date: formatDateOnly(item.transactionDate),
        category: item.type === TransactionType.LOAN_PAYMENT ? "Angsuran Masuk" : "Simpanan Masuk",
        direction: "Masuk",
        amount: asNumber(item.amount),
        memberName: item.member.user.name,
        description: item.description,
    }));
    const outgoingItems = loans.map((item) => ({
        code: item.loanCode,
        date: formatDateOnly(item.createdAt),
        category: "Pencairan Pinjaman",
        direction: "Keluar",
        amount: asNumber(item.principalAmount),
        memberName: item.member.user.name,
        description: `Pencairan pinjaman ${item.loanCode}`,
    }));
    return [...incomingItems, ...outgoingItems].sort((left, right) => right.date.localeCompare(left.date));
};
const matchesCashflowFilters = (item, filters) => {
...
const buildDelinquencyStatus = (loans) => {
    const today = getTodayRange().start;
    const hasDelinquent = loans.some((loan) => loan.status === LoanStatus.DELINQUENT || loan.nextDueDate < today);
    if (hasDelinquent)
        return "Menunggak";
    if (loans.length > 0)
        return "Lancar";
    return "Tanpa Pinjaman";
};
export const createAdminReportServices = (prisma = defaultPrisma) => ({
    async getSummaryReport(userId, filters) {
        ...
        const [activeMembers, inactiveMembers, savingsAggregate, activeLoansAggregate, totalTransactionAggregate, totalLoansDisbursedAggregate, installmentsTodayAggregate, loansDisbursedThisMonthAggregate, savingsInThisMonthAggregate, pendingApplications, dueToday, delinquentLoans, arrearsLoans, recentTransactions, latestApplications,] = await Promise.all([
        ...
        return {
            ...
            metrics: {
                ...
                totalSavings: asNumber(savingsAggregate._sum.amount),
                totalActiveLoans: asNumber(activeLoansAggregate._sum.remainingAmount),
                totalInstallmentsToday: asNumber(installmentsTodayAggregate._sum.amount),
                totalArrears: arrearsLoans.reduce((sum, loan) => sum + asNumber(loan.installmentAmount), 0),
                cashBalance: asNumber(totalTransactionAggregate._sum.amount) -
                    asNumber(totalLoansDisbursedAggregate._sum.principalAmount),
                loansDisbursedThisMonth: asNumber(loansDisbursedThisMonthAggregate._sum.principalAmount),
                savingsInThisMonth: asNumber(savingsInThisMonthAggregate._sum.amount),
            },
            quickStats: {
                pendingApplications,
                dueToday,
            ...
            latestApplications: latestApplications.map((item) => mapApplicationItem({
                ...item,
                status: item.status,
            })),
        };
    ...
    async getMembersReport(userId, filters) {
        ...
        const members = await prisma.memberProfile.findMany({
            where: {
                ...(filters.status && filters.status !== "Semua"
                    ? {
                        status: filters.status === "Aktif" ? MemberStatus.ACTIVE : MemberStatus.INACTIVE,
                    }
                    : {}),
        ...
        const items = members
            .map((member) => {
            const totalSavings = member.savingsBalances.reduce((sum, item) => sum + asNumber(item.amount), 0);
            const activeLoanAmount = member.loans.reduce((sum, loan) => sum + asNumber(loan.remainingAmount), 0);
    ...
    async getSavingsReport(userId, filters) {
        ...
        const filteredTransactions = transactions
            ...
            .filter((item) => !filters.savingsType || filters.savingsType === "Semua"
            ? true
        ...
        const memberTotals = members.map((member) => {
            ...
            for (const balance of member.savingsBalances) {
                ...
                if (name.includes("pokok"))
                    breakdown.pokok += amount;
                else if (name.includes("wajib"))
    ...
    async getLoansReport(userId, filters) {
        ...
        const items = loans
            ...
            principalAmount: asNumber(loan.principalAmount),
            remainingAmount: asNumber(loan.remainingAmount),
            installmentAmount: asNumber(loan.installmentAmount),
            tenor: loan.tenor,
    ...
    async getInstallmentsReport(userId, filters) {
        ...
        const filteredPayments = payments
            ...
            .filter((item) => {
            const queryMatches = containsQuery(item.loanCode, item.memberCode, item.memberName)(filters.query);
            const statusMatches = !filters.status || filters.status === "Semua" || filters.status === "Berhasil";
            return queryMatches && statusMatches;
        });
        const dueItems = loans
            .map((loan) => {
            const nextDueDate = formatDateOnly(loan.nextDueDate);
            const status = loan.status === LoanStatus.DELINQUENT || loan.nextDueDate < today.start
                ? "Menunggak"
        ...
        return {
            summary: {
                paymentsToday: asNumber(paymentsTodayAggregate._sum.amount),
                paymentsThisMonth: asNumber(paymentsThisMonthAggregate._sum.amount),
    ...
    async getArrearsReport(userId, filters) {
        ...
        const items = loans
            ...
            return {
                ...
                amountDue: asNumber(loan.installmentAmount),
                remainingAmount: asNumber(loan.remainingAmount),
                status: "Menunggak",
            };
        })
            .filter((item) => {
            const queryMatches = containsQuery(item.loanCode, item.memberCode, item.memberName)(filters.query);
            const bucketMatches = !filters.agingBucket ||
    ...
    async getCashflowReport(userId, filters) {
        ...
        const openingBalance = previousItems.reduce((sum, item) => sum + (item.direction === "Masuk" ? item.amount : -item.amount), 0);
        const cashIn = items
    ...
    async getDailyTransactionsReport(userId, filters) {
        ...
        const savingsIn = savingsTransactions.reduce((sum, item) => sum + asNumber(item.amount), 0);
        const installmentsPaid = loanPayments.reduce((sum, item) => sum + asNumber(item.amount), 0);
        const loansDisbursed = disbursedLoans.reduce((sum, item) => sum + asNumber(item.principalAmount), 0);
        return {
    ...
    async getMonthlyRecapReport(userId, filters) {
        ...
        const [savingsAggregate, loansAggregate, installmentsAggregate, newMembers, arrearsLoans,] = await Promise.all([
            prisma.transaction.aggregate({
                where: {
                    category: TransactionCategory.SAVINGS,
                    transactionDate: {
        ...
        const totalSavings = asNumber(savingsAggregate._sum.amount);
        const totalLoansDisbursed = asNumber(loansAggregate._sum.principalAmount);
        const totalInstallments = asNumber(installmentsAggregate._sum.amount);
        return {
            month: period.month,
            summary: {
                totalSavings,
    ...
    async getMemberDetailReport(userId, memberCode) {
        ...
        const activeLoans = member.loans.filter((loan) => ACTIVE_LOAN_STATUSES.includes(loan.status));
        ...
        return {
            ...
            summary: {
                ...
                totalLoans: member.loans.reduce((sum, loan) => sum + asNumber(loan.principalAmount), 0),
                activeLoanCount: activeLoanItems.length,
                activeLoanAmount: activeLoanItems.reduce((sum, loan) => sum + loan.principalAmount, 0),
                remainingLoan: activeLoanItems.reduce((sum, loan) => sum + loan.remainingAmount, 0),
                delinquencyStatus: buildDelinquencyStatus(activeLoans.map((loan) => ({
                    status: loan.status,
