import { LoanApplicationStatus, LoanStatus, MemberStatus, NotificationType, PaymentMethod, Role, TransactionCategory, TransactionType, } from "@prisma/client";
import { env } from "./server.config.env.js";
import { prisma as defaultPrisma } from "./server.db.js";
import { createAdminReportServices } from "./server.report-services.js";
import { AppError, addMonths, asNumber, comparePassword, computeInstallment, createJwt, formatDateOnly, hashPassword, maybeNull, normalizePhone, savePaymentProof, } from "./server.utils.js";
const ACTIVE_LOAN_STATUSES = [LoanStatus.ACTIVE, LoanStatus.DELINQUENT];
const MEMBER_LOAN_STATUS_FILTER = {
    in: ACTIVE_LOAN_STATUSES,
};
const uniqueCode = (prefix) => `${prefix}${Date.now()}${Math.floor(Math.random() * 1000)}`;
const toRole = (role) => (role === Role.ADMIN ? "admin" : "member");
const toMemberStatusLabel = (status) => status === MemberStatus.ACTIVE ? "Aktif" : "Nonaktif";
const toLoanApplicationStatusLabel = (status) => {
    if (status === LoanApplicationStatus.NEW)
        return "Baru";
    if (status === LoanApplicationStatus.UNDER_REVIEW)
        return "Ditinjau";
    if (status === LoanApplicationStatus.APPROVED)
        return "Disetujui";
    return "Ditolak";
};
const toLoanApplicationStatus = (status) => {
    if (status === "Ditinjau")
        return LoanApplicationStatus.UNDER_REVIEW;
    if (status === "Disetujui")
        return LoanApplicationStatus.APPROVED;
    return LoanApplicationStatus.REJECTED;
};
const toPaymentMethod = (method) => method === "Transfer" ? PaymentMethod.TRANSFER : PaymentMethod.CASH;
...
const mapLoan = (item, memberFacing = false) => ({
    ...
    remaining: asNumber(item.remainingAmount),
    installment: asNumber(item.installmentAmount),
...
const mapSavingsProduct = (item) => ({
    id: item.id,
    name: item.name,
    amount: asNumber(item.defaultAmount),
    isMandatory: item.isMandatory,
});
const mapLoanProduct = (item) => ({
    id: item.id,
    name: item.name,
    maxAmount: asNumber(item.maxAmount),
    interestRate: asNumber(item.interestRate),
    adminFeeRate: asNumber(item.adminFeeRate),
...
const buildSavingsSummary = (balances) => {
    const summary = {
        pokok: 0,
        wajib: 0,
        sukarela: 0,
...
const getMemberUser = async (prisma, userId) => {
    const user = await prisma.user.findUnique({
        where: { id: userId },
        include: { memberProfile: true },
    });
...
const nextMemberCode = async (prisma) => {
...
export const createServices = (prisma = defaultPrisma) => ({
    auth: {
        async login(input) {
            const user = await prisma.user.findUnique({
        ...
        async createLoanApplication(userId, input) {
            ...
            const application = await prisma.loanApplication.create({
                data: {
                    ...
                    estimatedInstallment: computeInstallment(input.amount, input.tenor, asNumber(loanProduct.interestRate)),
                },
                include: {
    ...
    admin: {
        ...
        async getDashboard(userId) {
            ...
            const [memberCount, savingsAggregate, loansAggregate, pendingApplications, activeLoansCount, dueToday, loanApplications, transactions,] = await Promise.all([
            ...
            return {
                ...
                stats: {
                    ...
                    totalLoans: asNumber(loansAggregate._sum.principalAmount),
                    pendingApplications,
                    activeLoansCount,
                    dueToday,
                },
        ...
        async updateMember(userId, memberId, input) {
            ...
            return {
                ...
                totalSavings: updated.memberProfile.savingsBalances.reduce((sum, item) => sum + asNumber(item.amount), 0),
            ...
            const transaction = await prisma.$transaction(async (tx) => {
                ...
                const updatedLoan = await tx.loan.update({
                    ...
                    data: {
                        ...
                        nextDueDate: nextStatus === LoanStatus.COMPLETED
                            ? loan.nextDueDate
                ...
                await tx.notification.create({
                    data: {
                        ...
                        message: nextStatus === LoanStatus.COMPLETED
