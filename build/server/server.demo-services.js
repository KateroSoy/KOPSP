...
const toApplicationStatusLabel = (status) => {
    if (status === "NEW")
        return "Baru";
...
const toLoanStatusLabel = (status, memberFacing = false) => {
...
const isActiveLoanStatus = (status) => status === "ACTIVE" || status === "DELINQUENT";
const getToday = () => formatDateOnly(new Date());
const getFutureDate = (months) => formatDateOnly(addMonths(new Date(), months));
...
const getUserOrThrow = (store, userId) => {
    const user = store.users.find((item) => item.id === userId);
    if (!user)
        throw new AppError(404, "USER_NOT_FOUND", "User was not found.");
    return user;
};
const getAdminOrThrow = (store, userId) => {
...
const getMemberOrThrow = (store, userId) => {
    const user = getUserOrThrow(store, userId);
    if (user.role !== "member") {
...
const nextCounter = (store, key) => {
    store.counters[key] += 1;
    return store.counters[key];
};
const nextMemberCode = (store) => `KSP-${String(nextCounter(store, "member")).padStart(5, "0")}`;
...
const mapLoan = (item, memberFacing = false) => ({
    id: item.id,
    memberId: item.memberId,
    name: item.name,
    amount: item.amount,
...
const findUserByPhone = (store, phone) => store.users.find((user) => normalizePhone(user.phone) === normalizePhone(phone));
const getSavingsSummary = (store, memberCode) => {
...
const listMemberTransactions = (store, memberCode) => store.transactions
    .filter((item) => item.memberCode === memberCode)
    .sort((left, right) => right.date.localeCompare(left.date))
    .map((item) => clone(item));
const listUserNotifications = (store, userId) => store.notifications
    .filter((item) => item.userId === userId)
    .sort((left, right) => right.date.localeCompare(left.date))
    .map(({ userId: _userId, ...item }) => clone(item));
const listAdminMembers = (store) => store.users
    ...
    hasActiveLoan: store.loans.some((loan) => loan.memberId === user.memberId && isActiveLoanStatus(loan.statusCode)),
    address: user.address,
    email: user.email,
}))
    .sort((left, right) => right.id.localeCompare(left.id));
const listAdminLoans = (store) => store.loans
    .slice()
    .sort((left, right) => right.id.localeCompare(left.id))
    .map((loan) => mapLoan(loan, false));
const fanOutAnnouncementNotifications = (store, announcement) => {
...
const isWithinRange = (value, startDate, endDate) => (!startDate || value >= startDate) && (!endDate || value <= endDate);
...
export const createDemoServices = () => {
    ...
    return {
        ...
        member: {
            async getDashboard(userId) {
                ...
                const activeLoan = store.loans.find((loan) => loan.memberId === user.memberId && isActiveLoanStatus(loan.statusCode));
                return clone({
                    user: {
                        id: user.id,
                        role: user.role,
            ...
            async createLoanApplication(userId, input) {
                ...
                if (store.loans.some((loan) => loan.memberId === user.memberId && isActiveLoanStatus(loan.statusCode))) {
                    throw new AppError(409, "ACTIVE_LOAN_EXISTS", "This member already has an active loan and cannot submit a new application.");
                }
                const loanProduct = (input.loanProductId
        ...
        admin: {
            async getDashboard(userId) {
                ...
                const pendingApplications = store.loanApplications.filter((item) => item.statusCode === "NEW" || item.statusCode === "UNDER_REVIEW");
            ...
            async createMember(userId, input) {
                ...
                store.memberBalances[memberCode] = Object.fromEntries(store.savingsProducts.map((product) => [product.id, 0]));
                return clone({
                    id: memberCode,
                    name: user.name,
                    phone: user.phone,
            ...
            async deleteMember(userId, memberId) {
                ...
                if (!user)
                    throw new AppError(404, "MEMBER_NOT_FOUND", "Member was not found.");
                const hasHistory = store.loans.some((loan) => loan.memberId === memberId) ||
                    store.loanApplications.some((item) => item.memberId === memberId) ||
                    store.transactions.some((item) => item.memberCode === memberId);
                if (hasHistory) {
                    throw new AppError(409, "MEMBER_HAS_HISTORY", "Members with transaction or loan history cannot be deleted. Set them inactive instead.");
                }
            ...
            async updateSavingsProduct(userId, productId, input) {
                ...
                if (!product)
                    throw new AppError(404, "PRODUCT_NOT_FOUND", "Savings product was not found.");
            ...
            async deleteSavingsProduct(userId, productId) {
                ...
                if (hasBalances || hasTransactions) {
                    throw new AppError(409, "PRODUCT_IN_USE", "Savings products with balances or transactions cannot be deleted.");
                }
            ...
            async listLoanProducts(userId) {
                getAdminOrThrow(store, userId);
                return clone(store.loanProducts.map((item) => ({
                    id: item.id,
                    name: item.name,
            ...
            async updateLoanProduct(userId, productId, input) {
                ...
                if (!product)
                    throw new AppError(404, "PRODUCT_NOT_FOUND", "Loan product was not found.");
                product.name = input.name.trim();
            ...
            async deleteLoanProduct(userId, productId) {
                ...
                if (hasApplications || hasLoans) {
                    throw new AppError(409, "PRODUCT_IN_USE", "Loan products with applications or loans cannot be deleted.");
                }
            ...
            async listLoanApplications(userId) {
                getAdminOrThrow(store, userId);
                return clone(store.loanApplications
                    .slice()
                    .sort((left, right) => right.date.localeCompare(left.date))
            ...
            async reviewLoanApplication(userId, applicationId, input) {
                ...
                if (input.status === "Ditinjau") {
                    ...
                    if (store.loans.some((loan) => loan.memberId === application.memberId && isActiveLoanStatus(loan.statusCode))) {
                        throw new AppError(409, "ACTIVE_LOAN_EXISTS", "This member already has an active loan and cannot receive another one.");
                    }
            ...
            async getSummaryReport(userId, filters) {
                ...
                const activeMembers = store.users.filter((user) => user.role === "member" && user.status === "Aktif").length;
                ...
                return clone({
                    ...
                    metrics: {
                        ...
                        cashBalance: store.transactions.reduce((sum, item) => sum + item.amount, 0) -
            ...
            async getMembersReport(userId, filters) {
                getAdminOrThrow(store, userId);
                const items = listAdminMembers(store)
                    .map((member) => {
                    const memberLoans = store.loans.filter((loan) => loan.memberId === member.id && isActiveLoanStatus(loan.statusCode));
                    const delinquencyStatus = memberLoans.some((loan) => loan.statusCode === "DELINQUENT")
                        ? "Menunggak"
                    ...
                    const matchesStatus = !filters.status || filters.status === "Semua" || item.status === filters.status;
                    const matchesLoanStatus = !filters.loanStatus ||
                        filters.loanStatus === "Semua" ||
                        (filters.loanStatus === "Ada Pinjaman" ? item.activeLoanCount > 0 : item.activeLoanCount === 0);
                    const matchesDelinquency = !filters.delinquencyStatus ||
            ...
            async getSavingsReport(userId, filters) {
                ...
                const transactions = store.transactions
                    ...
                    .filter((item) => !filters.memberCode ? true : item.memberCode === filters.memberCode)
                    .filter((item) => !filters.savingsType || filters.savingsType === "Semua"
                    ? true
                    : item.savingsType === filters.savingsType);
                const memberTotals = listAdminMembers(store)
            ...
            async getInstallmentsReport(userId, filters) {
                ...
                const payments = store.transactions
                    ...
                    .filter((item) => !filters.status || filters.status === "Semua" || filters.status === "Berhasil");
                const dueItems = store.loans
                    ...
                    .filter((item) => containsQuery(item.loanCode, item.memberCode, item.memberName)(filters.query))
                    .filter((item) => !filters.loanCode || item.loanCode === filters.loanCode.toUpperCase())
                    .filter((item) => !filters.status || filters.status === "Semua"
                    ? true
            ...
            async getArrearsReport(userId, filters) {
                ...
                const items = store.loans
                    ...
                    .filter((item) => containsQuery(item.loanCode, item.memberCode, item.memberName)(filters.query))
                    .filter((item) => !filters.agingBucket || filters.agingBucket === "Semua"
                    ? true
            ...
            async getCashflowReport(userId, filters) {
                ...
                const allItems = [
                    ...store.transactions.map((item) => ({
                        ...
                        date: item.date,
                        category: (item.category === "simpanan" ? "Simpanan Masuk" : "Angsuran Masuk"),
                        direction: "Masuk",
                        amount: item.amount,
                        memberName: item.memberName,
                ...
                const items = allItems.filter((item) => isWithinRange(item.date, range.startDate, range.endDate))
            ...
            async getDailyTransactionsReport(userId, filters) {
                ...
                const savingsItems = store.transactions.filter((item) => item.category === "simpanan" && item.date === selectedDate);
                const paymentItems = store.transactions.filter((item) => item.category === "pinjaman" && item.date === selectedDate);
            ...
            async getMemberDetailReport(userId, memberCode) {
                ...
                const paymentHistory = store.transactions
            ...
            async recordLoanPayment(userId, loanId, input) {
                ...
                store.notifications.unshift({
                    ...
                    userId: loan.userId,
                    title: "Pembayaran Berhasil",
                    message: loan.statusCode === "COMPLETED"
        ...
        notifications: {
            ...
            async markRead(userId, notificationId) {
                const notification = store.notifications.find((item) => item.id === notificationId && item.userId === userId);
                if (!notification) {
        ...
        transactions: {
            async list(userId, role) {
                if (role === "admin") {
                    getAdminOrThrow(store, userId);
                    return clone(store.transactions.slice().sort((left, right) => right.date.localeCompare(left.date)));
                }
                const user = getMemberOrThrow(store, userId);
